...
let pub = "846306316745834511"; //  PUBLİC KATAGORİ ID
let kayıt = "846306314837033000"   // KAYIT KATAGORİ ID
let terapi = "846306316078284812"  // TERAPİ KATEGORİ ID
let sorun = "846306316078284818"   // SORUN ÇÖZME KATAGORİ ID
let vk = "846306318763425797"   // VAMPİR KÖYLÜ KATAGORİ ID
let dc = "846306318763425792"  // DOĞRULUK CESARETLİK KATAGORİ ID
let game = "846306317962706984"  // OYUN KATAGORİ ID
let priv = "846306317169066053"    // PRİVATE KATAGORİ ID
let alone = "846306317962706991"   // ALONE KATAGORİ ID

//--------------------- CONSOL AKTİF OLDUĞUNA DAİR MESAJ GÖNDERME ------------------------------------\\

client.on ( "ready" , () => {
    console.log ( "Başarıyla", client.user.username + "İsmi İle Giriş Yapıldı!" );
    console.log("Asteroid Stat")
} );
...
//---------------------------------- BOTU SESLİ SOKMA ----------------------------------------\\
  ...
  client.on("ready", () => {
  client.channels.cache.get("846306315756371986").join();
  });

//---------------------------------- BOTU SESLİ SOKMA ----------------------------------------\\

 client.on("ready", async () => {
client.user.setPresence({ activity: { name: "✫ A S T E R O I D" }, status: "online" });
})
...
client.on ( "voiceChannelLeave" , ( member , channel ) => {
    if ( member.user.bot ) return
    if (!member.roles.cache.has ("846306314506076221")) return     // Yetkili Rol ID ( KOMUTU KULLANABİLCEK )
